...
rutaCart.post("/:id/productos", (req: Request, res: Response) => {
  if (carritoUsuario.length === 0) {
    ...
    const productoEncontrado = products.filter(
      (element) => element.id === id_prod
    );
    if (productoEncontrado[0]) {
      const carritoActualizado = carritoHerramientas.añadirProductoACarrito(
        id,
...
rutaCart.delete("/:id/productos/:id_prod", (req: Request, res: Response) => {
  ...
  const carritoActualizado = carritoHerramientas.eliminarProductoDeUnCarrito(
    id,
    id_productos
  );
  if (carritoActualizado) {
